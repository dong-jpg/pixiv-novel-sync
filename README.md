...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/health` | GET | 健康检查（uptime、DB 状态、运行中任务数） |
| `/api/dashboard/export/stats` | GET | 导出同步统计数据（JSON） |
| `/api/dashboard/settings/reload` | POST | 配置热重载 |
| `/api/dashboard/sync/<task_type>` | POST | 手动触发同步任务 |
| `/api/dashboard/logs` | GET | 查询任务日志 |

## 后续优化方向

### 功能增强
- [x] 小说/系列状态检查定时任务默认开启
- [x] 支持按作者批量同步指定用户的小说（用户详情页"备份全部小说"）
- [x] 支持导出同步统计数据（JSON）
- [x] 小说全文搜索（搜索标题/作者，支持排序）
- [ ] 支持多账号管理

### 性能优化
- [ ] 批量事务支持（减少 SQLite commit 次数）
- [x] Database 类实现上下文管理器协议
- [x] 已完成任务的内存清理（`_jobs` 字典限制 100 个）
- [x] 并发同步限制改为信号量

### 前端改进
- [x] 小说列表页支持搜索和排序
- [x] 用户详情页支持直接触发同步
- [x] 日志页面支持实时刷新（10 秒自动刷新开关）
- [x] 移动端适配优化
