...
class SyncSettings:
    ...
    # 收藏列表专用翻页上限。None = 跟随 max_pages_per_run（旧行为）。
    ...
    # 注意这里只有「已确认记录保留天数」。曾经还有一个 pending_deletion_grace_period_days
    # （「待确认宽限天数」），但 cleanup_old_pending_deletions 早就不按时间自动确认
    # pending 记录了（会让待确认列表静默消失），那个字段因此长期无人读取、只在设置页
    # 里误导用户，已于 2026-09-03 移除。别再加回来。
