...
## 18. 当前实现状态（2026-08-13）

本节把需求验收与当前代码状态分开记录，避免把“页面入口存在”误读为整项需求已完成。细节和缺口证据见 [docs/AUDIT_REPORT_2026-08-13.md](AUDIT_REPORT_2026-08-13.md)。

| 能力 | 状态 | 当前事实 |
|---|---|---|
| 本地偏好统计与增量分析 | `PARTIAL` | 已有本地统计、版本化画像和共享 job；分析范围目前主要是最小正文长度/批量参数，收藏、追更、作者、标签、时间和失效筛选未完整接入。 |
| AI 偏好总结 | `MISSING` | 当前 AI job 只清洗关键词；结构化偏好总结、画像摘要和置信度生成尚未接入。 |
| 画像保存、默认、删除 | `IMPLEMENTED` | 现有 API、存储和测试覆盖基础 CRUD。 |
| 搜索计划生成 | `PARTIAL` | 可基于画像临时生成并去重查询；独立编辑、保存、版本和完整 stream API 尚未提供。 |
| 推荐搜索、过滤、打分 | `PARTIAL` | Pixiv 搜索、字数/归档过滤、系列去重和可解释规则已实现；失败/取消发布隔离仍缺失。 |
| AI 推荐解释 | `MISSING` | 当前理由由确定性规则拼接，未调用 AI 生成结构化解释。 |
| 推荐反馈与屏蔽 | `PARTIAL` | 感兴趣、不感兴趣和屏蔽作者已有基础；屏蔽标签、待阅读/待同步、立即同步和完整反馈 CRUD 尚未覆盖。 |
| AI 创作偏好注入 | `PARTIAL` | 主要创作路径已有偏好上下文；成人局部润色请求字段尚未进入阅读页或成人 Prompt。 |
| 推荐失败/取消语义 | `MISSING` | run 在成功前逐条写入正式结果，取消还可能被统一任务标记为 succeeded；需要 staging/发布事务和端到端回归测试。 |

README 中的“智能推荐”描述应与本表同步更新；新增能力必须先更新本节、统一需求和对应测试，再更新用户入口文档。
