...
- **已进入「待确认删除」的条目不收录**：你自己取消收藏 / 取消追更的作品会进待确认列表（`/dashboard/pending-deletions`）。此时该条目以「等你决定」为准，不会同时挂进救援目录——否则一个被本人取消、又恰好被 Pixiv 下架的作品会两边都出现，救援页还给它标着「我的收藏」，看起来就像「我取消的东西被算成拯救」。只有 `pending` 状态排除；点「确认移除」会真删本地备份（自然从目录消失），点「忽略恢复」表示你要留着（照常参与救援判定）。人工 `include` 纠错优先于这条排除。

### 来源标注：哪些是你真正收藏 / 追更过的

救援目录里的每个条目会标注它是通过哪些渠道进入备份的。来源分两类：

- **本人主动关联**：`我的收藏`（bookmark）、`我的追更`（subscribed_series）——卡片右上角带「我收藏/追更过」徽标（API 字段 `personal_relation: true`）；
- **批量扫描**：`关注用户`（following_user_scan）、`用户备份`（user_backup）、其他——本人未必在乎，只是顺着关注作者或全量备份抓下来的。

区分这两类的意义：生产实测救援成功的条目里 `bookmark` 来源为 0，全部来自批量扫描。不标出来，你没法一眼分辨哪些是自己真正在意的丢失内容。
