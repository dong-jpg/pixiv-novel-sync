...
`GET /api/dashboard/logs?category=sync|ai` is the only place they meet: it reads whichever table matches the category and projects `ai_jobs` into the same shape. Nothing merges them in storage. The scheduler loop trims both to `sync.task_log_retention_days` (default 14; `cleanup_old_task_logs` / `cleanup_ai_jobs` share the one value, because the log page presents the two tables as one page with two categories). What lands in `task_logs.stats_json` is capped at 8 KB by `webapp.py:_prune_stats_for_log` — size-based, not a field blacklist, with a `_pruned` receipt naming what was dropped; `recommendation_run` used to write 351 KB per round by embedding its whole candidate list. Only the persisted copy is pruned, never the in-memory `JobState.stats` the live progress endpoint reads.
...
The web layer runs jobs on daemon threads and mirrors state into `task_logs` (`_submit_shared_job` / `_run_shared_web_job`), enforcing a single active job via `_has_any_running_web_job()` inside the `JobManager` lock (closing a TOCTOU window). A succeeded job whose stats carry `aborted_reason` or `truncated` is written as **`partial`**, not `succeeded` (`_task_log_status_for_stats`) — this exists because a rate-limited status check once reported green while checking 30 of 800 novels. A bare `incomplete` also counts as `partial`; the one exception is `rotation_pending`, which marks "not finishing is by design" (following-author rotation covers `users_limit` authors per round, ~12.8 days per full cycle, so every round is `incomplete`) and stays green. The direction matters: new `stats["incomplete"] = True` sites default to `partial` and only an explicit `rotation_pending` downgrades them, so amber stays rare enough to mean something — `tests/test_sync_engine_incremental.py` greps `sync_engine.py` to make sure every `incomplete` site declares which of the three it is. Relatedly, `web/utils.py:_classify_pixiv_response` is three-state (`ok` / `missing` / `unknown`): only an explicit "does not exist" marks something deleted; rate limits fall through to `unknown` and leave the record alone.
...
Rotation slots — not time — are the scarce resource in `sync_following_novels`: `users_limit` authors per round, so a wasted slot delays everyone behind it. `_order_following_users_for_rotation` therefore sorts on `(deprioritized, has_timestamp, timestamp)`, where authors confirmed `no_novels` (Pixiv follows don't separate illustration from novels; production had 87 of 298) are pushed to the back for `NO_NOVELS_RECHECK_DAYS`. Deprioritize, never exclude: a missing timestamp yields an age of infinity so a never-scanned empty author still gets one pass, and after the window they rejoin the normal order. Progress is reported as `rotation_never_synced` / `rotation_oldest_age_days`; `users_remaining` is `len(candidates) - users_limit`, constant every round, and must not be used as a progress indicator.
...
Two eligibility rules are easy to get wrong because they look like they belong to the same subsystem but don't. First, the catalog **excludes anything with a `pending` row in `pending_deletions`**: unbookmarking / unsubscribing writes only that row (it neither clears the `bookmark_*` `sources` rows nor flips `series.is_subscribed` — both wait for the user's "确认移除"), so an item the user removed themselves *and* that Pixiv also took down would otherwise sit in both lists with the rescue page still labelling it "我的收藏". Only `pending` excludes: `confirmed` really deletes the local rows, `restored` is the user explicitly keeping it. A manual `include` override beats the exclusion. Second, `personal_relation` on list items separates "you bookmarked/subscribed this" from "a bulk scan happened to grab it" — measured on production, all 365 rescue successes came from `following_user_scan` / `user_backup` with zero `bookmark` sources, so without the flag there's no way to tell which losses you actually cared about. The read API (`get_rescue_novel` / `get_rescue_series`, what the userscript calls by ID) is deliberately *not* subject to the pending exclusion: the catalog decides what to browse, the API serves the backup for an ID you already have.
...
Settings is five first-class pages (`/dashboard/settings/{sync,models,agents,adult,system}`, `dashboard_settings_*.html` plus the shared `dashboard_settings_nav.html`); bare `/dashboard/settings` 302s to `sync`. The old single 124 KB template is gone — six test files assert on the new paths, so a further split means migrating those assertions in the same commit. Only `sync` and `system` have field partitions in `web/managers.py:SETTINGS_SECTIONS` (saved via `PUT /api/dashboard/settings/<section>`, which writes only that partition and leaves the rest of the YAML alone); the other three pages configure database-backed AI state through the existing `ai_web.py` endpoints, so `PUT /api/dashboard/settings/models` deliberately 400s. `SETTINGS_SECTIONS` must stay a total partition of `web/utils.py:_settings_to_dict` — a field in no section can never be saved, and `tests/test_settings_sections.py` asserts both directions. The three AI settings pages (`models`/`agents`/`adult`) share `dashboard_ai_health_band.html`, a read-only config-health banner driven by a zero-network projection (`GET /api/dashboard/ai/health`).

AI 创作 got the same treatment: the 1946-line `dashboard_ai.html` is gone, replaced by `/dashboard/ai` (project list, `dashboard_ai_projects.html`) plus three project pages `/dashboard/ai/projects/<id>[/chapters|/notes]`, which share the Jinja partial `dashboard_ai_project_nav.html`. `project_id` comes from the route, not from JS parsing `location.search` — templates write `const projectId = {[ project_id ]};`, and all three pages go through `ai_web.py:_require_project` (missing project ⇒ 404, not a blank page whose every request fails). The legacy deep link `/dashboard/ai?project_id=<id>` still 302s to the project page for positive integers and falls back to the list for anything else. Two couplings survive the split and are asserted across files by `tests/test_frontend_library_os.py`: the chapters page must idempotently re-save style control and profile bindings before starting a pipeline (because `ai/services/projects.py:_project_style_control_prompt` reads the *persisted* project row, while the edit UI lives on the project page), and `tests/test_ai_page_routes.py` greps every `@event` binding in each page's markup — including `{% include %}`d partials — against what its `setup()` actually returns, since `window.initVueApp` has no fallback and an unexported handler fails silently.
