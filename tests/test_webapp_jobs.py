...
def test_scheduler_job_spec_uses_shared_source_and_type():
    scheduler_job_spec = getattr(webapp_module, "_scheduler_job_spec", None)
    assert callable(scheduler_job_spec)
    spec = scheduler_job_spec("pending_deletion_detection")
...
@pytest.mark.parametrize(
    ("scheduled_name", "task_type"),
    [("bookmarks", "bookmark"), ("following_list", "following_users")],
)
def test_scheduler_job_spec_normalizes_legacy_schedule_names(scheduled_name, task_type):
    spec = webapp_module._scheduler_job_spec(scheduled_name)

    assert spec.task_types == [task_type]
...
def test_stopping_scheduler_requests_shared_cancel_for_current_job():
    cancelled = []
    scheduler = AutoSyncScheduler(
        None,
        None,
...
def test_stop_during_submit_cancels_job_before_runner_starts():
    submit_entered = threading.Event()
    release_submit = threading.Event()
    calls = []
    state = SimpleNamespace(job_id="shared-1", status=JobStatus.QUEUED)

    def submit_task(settings, task_name):
        submit_entered.set()
    ...
    worker = threading.Thread(
        target=scheduler._run_single_task,
        args=(object(), "bookmarks"),
        daemon=True,
    )
    worker.start()
...
def test_app_scheduler_submits_and_runs_shared_job_synchronously(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(webapp_module, "Database", RecordingDatabase)
    monkeypatch.setattr(webapp_module.JobRunner, "run", successful_run)
    app = _app(tmp_path, monkeypatch)
    scheduler = app.config["auto_sync_scheduler"]
    scheduler._running = True
    settings = SimpleNamespace(storage=SimpleNamespace(db_path=tmp_path / "scheduler.db"))

    scheduler._run_single_task(settings, "pending_deletion_detection")

    job = app.config["job_manager"].latest_job()
    assert job is not None
    assert job.spec.source == JobSource.SCHEDULER
    assert job.spec.task_types == ["pending_deletion_detection"]
    assert job.status == JobStatus.SUCCEEDED
    assert RecordingDatabase.created_logs[0]["is_auto_sync"] is True


def test_auto_sync_status_reads_current_job_from_shared_manager(tmp_path, monkeypatch):
    app = _app(tmp_path, monkeypatch)
    manager = app.config["job_manager"]
    job = manager.submit(webapp_module._scheduler_job_spec("bookmarks"))
    monkeypatch.setattr(
        AutoSyncScheduler,
        "get_status",
        lambda self: {"running": True, "current_task_job_id": job.job_id},
    )

    response = app.test_client().get("/api/dashboard/auto-sync/status")
...
def test_health_counts_running_jobs_from_shared_manager(tmp_path, monkeypatch):
    ...
    assert manager.mark_running(job.job_id)
...
def test_scheduler_retries_failed_initialization_after_stop_and_restart(
    ...
    try:
        assert scheduler._current_task_job_id is None
...
def test_create_app_shares_scheduler_owner_by_normalized_db_path_and_releases_on_stop(
    ...
    try:
        ...
        owner_manager = first_app.config["job_manager"]
        assert second_app.config["job_manager"] is owner_manager
        owner_job = owner_manager.submit(webapp_module._scheduler_job_spec("bookmarks"))
        assert owner_manager.mark_running(owner_job.job_id)
        ...
        owner_manager.mark_succeeded(owner_job.job_id)
        first_owner.stop()
...
def test_scheduler_registry_keeps_stopping_owner_until_worker_exits(tmp_path):
    ...
    manager = webapp_module.JobManager()
    ...
    try:
        ...
        while_stopping, created_while_stopping = webapp_module._get_or_create_scheduler_owner(
            ...
            config_path=None,
            env_path=None,
            job_manager=webapp_module.JobManager(),
        )
